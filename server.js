...
app.get('/delete/:id', (req, res) => {
    //Getting the document into "image"
    let image = LTPP.find({ _id: req.params.id });
    //Deleting the image from the directory and then deleting it as well from the db as well in the call back
    fs.unlink(image[0].filePath, (err) => {
        if (err) {
            throw err;
        } else {
            //Deleting the document from the database after successfully deleting the image
